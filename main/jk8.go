...
func main() {
	...
	//笔记:
	//比较Ring和List的五大不同的罪状
	// 1.结构复杂度不同,Ring数据结构由它自身代表,而List类型是由Element+len联合构成.
	// 2.表示纬度上,Ring类型的值严格来讲,只代表所属循环链表中的一个元素，而一个List则代表了一个完整的链表.
	// 3.两个结构体的初始化a,初始化Ring,可以指定数量,并且指定以后不可以变化,而List不可以指定数量,而且是可变的.
	// 4.两个结构体的初始化b,var初始话的时候Ring是一个长度为1的环形链表,而List则是一个长度为0的双向链表.
	// 5.查找长度的性能上比较,对于Ring来说,len的查找的时间复杂度是O(N),而List的时间时间复杂度为O(1)--其实也是得益于数据构成
	//拓展,没有银弹指的是:没有任何一项技术或方法可以使软件工程的生产力在10年内提高10倍
}
